...
conn = psycopg2.connect("dbname='culturalmapper_Istanbul' user='culturalmapper' host='culturalmapper-la.cbjpxqmibsmf.us-east-1.rds.amazonaws.com' password='UVAdsi2017'")
...
class listener(StreamListener):
    def on_data(self, status):
        ...
        while True:
            try:
                cur = conn.cursor()
                command = ("INSERT INTO istanbul_city_primary(id, created_at, source, text, text_lang, user_id, user_location, user_handle, user_lang ) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % (d['id'], (datetime.datetime.strptime(d['created_at'], '%a %b %d %H:%M:%S +0000 %Y')), d['source'], d['text'].replace("'", ""), d['lang'], d['user']['id'], d['user']['location'], d['user']['screen_name'], d['user']['lang']))
                cur.execute(command)
                conn.commit()
                cur.close()
            ...
            try:
                if d['coordinates'] is not None:
                    cur = conn.cursor()
                    command = ("INSERT INTO istanbul_city_secondary( id, long, lat) VALUES ('%s', '%s', '%s');" % (d['id'], d['coordinates']['coordinates'][0], d['coordinates']['coordinates'][1]))
                    cur.execute(command)
                    conn.commit()
                    cur.close()
            except BaseException as e:
            ...
            try:
                if d['user']['description'] is not None:
                    cur = conn.cursor()
                    command = ("INSERT INTO istanbul_user_desc(id, user_id, user_desc) VALUES ('%s','%s','%s');" % (d['id'], d['user']['id'], d['user']['description'].replace("'", " ")))
                    cur.execute(command)
                    conn.commit()
                    cur.close()
            except BaseException as e:
