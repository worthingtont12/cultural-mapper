"""This is a listener for tweets in Chicago."""
import datetime
import time
import sys
import json
...
class listener(StreamListener):
    ...
    def on_data(self, status):
        ...
        while True:
            try:
                cur = conn.cursor()
                command = ("INSERT INTO chicago_city_primary(id, created_at, source, text, text_lang, user_id, user_location, user_handle, user_lang ) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % (
            ...
            try:
                if d['coordinates'] is not None:
                    cur = conn.cursor()
                    command = ("INSERT INTO chicago_city_secondary( id, long, lat) VALUES ('%s', '%s', '%s');" % (
            ...
            try:
                if d['is_quote_status'] is True:
                    cur = conn.cursor()
                    command = ("INSERT INTO chicago_quoted(id, q_id, q_created_at, q_text, q_text_lang, q_user_id, q_user_location, q_user_handle, q_user_lang ) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % (d['id'], d['quoted_status']['id'], (datetime.datetime.strptime(d['quoted_status'][
            ...
            try:
                if d['user']['description'] is not None:
                    cur = conn.cursor()
                    command = ("INSERT INTO chicago_user_desc(id, user_id, user_desc) VALUES ('%s','%s','%s');" % (
