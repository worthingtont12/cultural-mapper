...
temp <- clean.topics %>%
  ...
  group_by(lang.topic, time = as.Date(tz)) %>% 
  count() %>% 
  spread(lang.topic, n) %>%
  arrange(time)
...
# This will run the model, but cannot capture the standard out.
models <- apply(temp[,2:3],MARGIN = 2, auto.arima,trace=T,stepwise = F, parallel = T)
...
models <- apply(temp_filtered[,-1], 2, arimaTrace)

# 7 day average
ts.plot(temp$Arabic)
lines(stats::filter(temp$Arabic,rep(1/7,7)), col='red')

apply(temp[,2:3],2, stats::filter, rep(1/7,7))

models[1]
models["English"]

# 2nd model, coefficients
models[[2]][1]
c<-models[[2]][1]
...
write.csv(coefs,"smooth_model_coefficients.csv")
